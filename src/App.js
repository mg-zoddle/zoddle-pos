...
const QrCode = ({ className }) => (
  <Icon className={className}>
    <rect x="3" y="3" width="5" height="5" rx="1" ry="1" />
    <rect x="16" y="3" width="5" height="5" rx="1" ry="1" />
    <rect x="3" y="16" width="5" height="5" rx="1" ry="1" />
...
const API_ENDPOINT =
  "https://script.google.com/macros/s/AKfycbx6uDkPFPEBGmwQ8LC7HsPnvGHbcIqFCiYBbh9r-o8W8I2fLVkBGFNkf_VXs05ADXVF/exec";

// --- SECURITY CONFIGURATION ---
const APP_PIN = "2026"; // 🔒 CHANGE THIS to your desired passcode
...
export default function App() {
  ...
  const fetchInventory = async () => {
    try {
      ...
      if (cached) {
        ...
        setStatus({
          type: "warning",
          text: "Offline Mode: Using cached inventory",
        });
      } else {
  ...
  const syncOfflineQueue = async () => {
    ...
    setStatus({
      type: "warning",
      text: `Syncing ${syncQueue.length} offline orders...`,
    });
    ...
    setStatus({
      ...
      text:
        newQueue.length === 0
          ? "All offline orders synced!"
          : `${newQueue.length} orders failed to sync.`,
    });
  };
  ...
  const handleSkuSearch = (e) => {
    ...
    const match = inventory.find(
      (i) => i.sku?.toString().toUpperCase() === sku
    );
    if (match) {
      setItemForm((prev) => ({
        ...prev,
  ...
  const addToCart = () => {
    if (!itemForm.sku || !itemForm.zrp)
      return alert("SKU and ZRP are required.");
    setCart([
      ...cart,
      {
        ...itemForm,
        mrp: parseFloat(itemForm.mrp) || 0,
        zrp: parseFloat(itemForm.zrp),
      },
    ]);
    setItemForm({
      sku: "",
  ...
  const calculateTotals = () => {
    ...
    if (code === "ZODDLE") discount = gross * (totalUnits > 3 ? 0.2 : 0.1);
    else if (code === "ZOD30") discount = gross * 0.3;
    else if (code === "ZOD200")
      discount = 200 + (totalUnits >= 4 ? (gross - 200) * 0.1 : 0);
    else if (code === "B2G1") {
      let flatList = [];
      cart.forEach((i) => {
        for (let u = 0; u < i.units; u++) flatList.push(i.zrp);
      });
      flatList.sort((a, b) => b - a);
      if (flatList.length >= 3) {
        discount += flatList[2];
        if (flatList.length >= 4)
          discount += flatList.slice(3).reduce((a, b) => a + b * 0.1, 0);
      }
  ...
  const submitOrder = async () => {
    ...
    if (isOnline) {
      try {
        ...
        setStatus({
          type: "warning",
          text: "Network error. Saved offline securely.",
        });
        resetForm();
      }
    } else {
        ...
        <div className="max-w-md w-full bg-white p-8 rounded-3xl shadow-lg text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-2">
              Enter PIN to access POS & Decrypt Data
            </p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            />
            {authError && (
              <p className="text-red-500 text-sm font-bold">Incorrect PIN</p>
            )}
            <button
              type="submit"
              className="w-full bg-pink-600 hover:bg-pink-700 text-white font-bold p-4 rounded-xl shadow-md transition-colors"
            >
              Unlock Terminal
            </button>
          </form>
    ...
    <div className="min-h-screen bg-gray-50 pb-24 text-gray-800 font-sans md:p-4">
      ...
      <div className="bg-white shadow-sm sticky top-0 z-40">
        <div className="p-4 flex justify-between items-center max-w-4xl mx-auto">
          <div>
            <h1 className="text-xl font-bold text-pink-700 leading-tight">
              Zoddle POS
            </h1>
        ...
        {status.text && status.type !== "info" && (
          <div
            className={`px-4 py-2 text-xs text-center font-bold ${
              status.type === "error"
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-6">
        ...
        <section className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-gray-700 flex items-center gap-2">
              <FileText className="w-5 h-5 text-pink-500" /> Order Details
            </h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border-0 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-pink-200"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCustomer({ ...customer, executive: e.target.value })
                  }
                  className="w-full bg-gray-50 border-0 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-pink-200"
                  placeholder="Required"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCustomer({ ...customer, discountCode: e.target.value })
                  }
                  className="w-full bg-pink-50 text-pink-700 font-bold uppercase border-0 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-pink-200"
                  placeholder="Code"
                />
              </div>
            </div>
        ...
        <section className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-gray-700 flex items-center gap-2">
              <Search className="w-5 h-5 text-pink-500" /> Scan / Add Item
            </h2>
            {itemForm.sku && (
              <span
          ...
          <div className="space-y-4">
            <input
              ...
              className="w-full bg-pink-50 border border-pink-100 rounded-xl p-3 font-mono text-lg uppercase focus:ring-2 focus:ring-pink-300"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className={`w-full rounded-lg p-2 text-sm border-0 ${
                    isSkuLocked
                      ? "bg-gray-100 text-gray-500"
                      : "bg-gray-50 focus:ring-2 focus:ring-pink-200"
                  }`}
                />
              </div>
              <div>
              ...
              {!isSkuLocked && (
                <div>
                  ...
                  <input
                    ...
                    value={itemForm.mrp}
                    onChange={(e) =>
                      setItemForm({ ...itemForm, mrp: e.target.value })
                    }
                    className="w-full bg-gray-50 border-0 rounded-lg p-2 text-sm"
                    placeholder="₹"
                  />
                </div>
              )}
              <div className={isSkuLocked ? "col-span-2" : ""}>
                <label className="text-[10px] font-bold text-pink-500 uppercase">
                  ZRP Price
                </label>
                <input
                  type="number"
                  ...
                  readOnly={isSkuLocked}
                  onChange={(e) =>
                    setItemForm({ ...itemForm, zrp: e.target.value })
                  }
                  className={`w-full font-bold text-pink-600 rounded-lg p-2 text-sm border-0 ${
                    isSkuLocked
                      ? "bg-gray-100"
            ...
            {!isSkuLocked && itemForm.sku && (
              <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-dashed border-gray-300">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 flex-1 bg-white border border-gray-200 p-2 rounded-lg text-sm text-gray-600 font-medium"
                >
                  <Camera className="w-4 h-4" />{" "}
                  {itemForm.photoData ? "Change Photo" : "Take Photo"}
                </button>
                {itemForm.photoData && (
                  <img
                    src={itemForm.photoData}
                    alt="Preview"
            ...
            <button
              onClick={addToCart}
              className="w-full bg-gray-900 text-white font-bold p-3 rounded-xl shadow-md active:scale-[0.98] transition-transform"
            >
              Add to Cart
            </button>
          </div>
        ...
        {cart.length > 0 && (
          <section className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
              <h2 className="font-bold text-gray-700 flex items-center gap-2">
                <ShoppingCart className="w-5 h-5 text-pink-500" /> Cart Items
              </h2>
              ...
              {cart.map((item, idx) => (
                <div key={idx} className="p-4 flex gap-3 items-center">
                  {item.photoData ? (
                    <img
                      src={item.photoData}
                      className="w-12 h-12 rounded object-cover border bg-gray-100 flex-shrink-0"
                      alt="Item"
                    />
                  ) : (
                    <div className="w-12 h-12 rounded bg-gray-100 border flex items-center justify-center text-[8px] text-gray-400 text-center flex-shrink-0">
                      NO IMG
                    </div>
                  )}
          ...
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom-10">
            ...
            <div className="overflow-y-auto p-6 space-y-6 flex-1">
              ...
              <div className="bg-gray-50 p-4 rounded-2xl text-sm space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">{customer.executive}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Items</span>
                  <span className="font-bold">{totals.totalUnits} Units</span>
                </div>
              </div>
              <div className="flex flex-col items-center p-6 border-2 border-dashed border-pink-100 bg-pink-50/50 rounded-3xl">
                ...
                <p className="text-xs font-bold text-pink-700 uppercase tracking-wider mb-4">
                  Scan to Pay
                </p>
                <img
                  src="https://cdn.shopify.com/s/files/1/0924/1041/3419/files/Zoddle_-_QR_Code.png?v=1771588559"
                  alt="UPI QR"
                  className="w-40 h-40 rounded-xl shadow-sm mix-blend-multiply"
                />
              </div>
            </div>
            <div className="p-4 border-t bg-white">
              <button
                onClick={submitOrder}
                disabled={isSaving}
                className="w-full bg-green-500 hover:bg-green-600 text-white font-bold p-4 rounded-xl shadow-lg shadow-green-200 text-lg flex justify-center items-center gap-2"
              >
                {isSaving
                  ? "Saving..."
                  : isOnline
